#!/usr/bin/env python3
"""Deploy contracts/insureshield.py to GenLayer StudioNet and record the
...
- refuses to deploy unless contracts/insureshield.py is committed and
        ...
        try:
            request = urllib.request.Request(
                RPC, data=body, headers={"Content-Type": "application/json",
                                         "User-Agent": "insureshield-deploy"})
...
def main() -> None:
    ...
    if git("status", "--porcelain", "--", "contracts/insureshield.py"):
        raise SystemExit("refusing: contracts/insureshield.py has uncommitted changes")
    commit = git("log", "-1", "--format=%H", "--", "contracts/insureshield.py")
    head = git("rev-parse", "HEAD")
    blob = git("rev-parse", "HEAD:contracts/insureshield.py")
