...
MUTATIONS = [
    ...
    ("over-long quotes cut mid-word",
     "        text = cut[:cut.rfind(\" \")].strip() if \" \" in cut else \"\"\n",
     "        text = cut\n"),
    ("duplicate quotes kept",
     "            if grounded is not None and grounded not in quotes:\n",
     "            if grounded is not None:\n"),
    ("a missing section makes the answer invalid",
     "    return {n: _section(raw.get(n)) for n in names}\n",
     "    if not all(isinstance(raw.get(n), dict) for n in names):\n        return None\n    return {n: _section(raw.get(n)) for n in names}\n"),
    ("an answer with no sections is accepted",
     "        if len(inner) != 1:\n            return None\n",
